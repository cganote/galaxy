define("mvc/history/history-contents", ["exports", "mvc/base/controlled-fetch-collection", "mvc/history/hda-model", "mvc/history/hdca-model", "mvc/history/history-preferences", "mvc/history/job-states-model", "mvc/base-mvc", "utils/ajax-queue", "libs/underscore", "libs/backbone"], function(exports, _controlledFetchCollection, _hdaModel, _hdcaModel, _historyPreferences, _jobStatesModel, _baseMvc, _ajaxQueue, _underscore, _backbone) {
    ...
    var limitPerPageDefault = window.localStorage.getItem("historyContentsLimitPerPageDefault") || 500;
    ...
    var HistoryContents = _super.extend(_baseMvc2.default.LoggableMixin).extend({
        ...
        fetchHidden: function fetchHidden(options) {
            ...
            return this.fetch(options).always(function() {
                _this3.trigger("fetching-hidden-done", _this3);
            });
        },
        ...
        _filterAndUpdate: function _filterAndUpdate(filterParams, updateWhat) {
            ...
            return this.fetch({
                ...
                fetched = fetched.reduce(function(modelArray, currJson, i) {
                    var model = _this4.get(currJson[idAttribute]);
                    return model ? modelArray.concat(model) : modelArray;
                }, []);
                return _this4.ajaxQueue("save", updateArgs, fetched);
            });
        },
        ...
        _recursivelyFetch: function _recursivelyFetch(options, detailKeys, deferred, limit, offset) {
            var _this5 = this;
            ...
            _.defer(function() {
                _this5.fetch.call(_this5, _options).fail(deferred.reject).done(function(response) {
                    deferred.notify(response, limit, offset);
                    if (response.length !== limit) {
                        _this5.allFetched = true;
                        deferred.resolve(response, limit, offset);
                    } else {
                        _this5._recursivelyFetch(options, detailKeys, deferred, limit, offset + limit);
                    }
                });
            });
        },
        ...
        progressivelyFetchDetails: function progressivelyFetchDetails(options) {
            ...
            this._recursivelyFetch(options, _hdaModel2.default.HistoryDatasetAssociation.prototype.searchAttributes.join(","), deferred, options.limitPerCall || this.limitPerProgressiveFetch);
            return deferred;
        },
