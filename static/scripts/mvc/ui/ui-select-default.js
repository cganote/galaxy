define("mvc/ui/ui-select-default", ["exports", "utils/utils", "mvc/ui/ui-buttons"], function(exports, _utils, _uiButtons) {
    "use strict";
    ...
    var View = Backbone.View.extend({
        initialize: function initialize(options) {
            ...
            this.on("change", function() {
                if (_this.model.get("onchange")) {
                    _this.model.get("onchange")(_this.value());
                }
            });
        ...
        _renderClassic: function _renderClassic() {
            ...
            if (this.model.get("multiple")) {
                ...
                this.$resize.removeClass().addClass("icon-resize").show().off("mousedown").on("mousedown", function(event) {
                    ...
                    var currentHeight = _this3.$select.height();
                    _this3.minHeight = _this3.minHeight || currentHeight;
                    $("#dd-helper").show().on("mousemove", function(event) {
                        _this3.$select.height(Math.max(currentHeight + (event.pageY - currentY), _this3.minHeight));
        ...
        _renderSearchable: function _renderSearchable() {
            ...
            if (!this.model.get("multiple")) {
                this.$dropdown.show().on("click", function() {
                    if (_this4.$select.select2) {
                        _this4.$select.select2("open");
                    }
                });
            }
            ...
            if (this.model.get("multiple") && !this.model.get("individual") && !this.model.get("readonly")) {
                this.all_button = new _uiButtons2.default.ButtonCheck({
                    onclick: function onclick() {
                        var new_value = [];
                        if (_this4.all_button.value() !== 0) {
                            _.each(_this4.model.get("data"), function(option) {
                                new_value.push(option.value);
                            });
                        }
                        _this4.value(new_value);
            ...
            if (this.model.get("searchable")) {
                ...
                this.$select.select2({
                    ...
                    query: function query(q) {
                        _this5.matched_tags = {};
                        var pagesize = _this5.model.get("pagesize");
                        var results = _.filter(_this5.data2, function(e) {
                            var found = false;
                            _.each(e.tags, function(tag) {
                                if (_this5._match(q.term, tag)) {
                                    found = _this5.matched_tags[tag] = true;
                                }
                            });
                            return found || _this5._match(q.term, e.text);
                        });
                        q.callback({
                    ...
                    formatResult: function formatResult(result) {
                        var extraTagWarning = "";
                        var filteredTags = _.filter(result.tags, function(t) {
                            return _this5.matched_tags.hasOwnProperty(t);
                        });
                        if (filteredTags.length > 5) {
                            extraTagWarning = "&nbsp;<div class=\"label label-warning\">" + (filteredTags.length - 5) + " more tags</div>";
                        }
                        return "\n                    " + _.escape(result.text) + "\n                    <div class=\"ui-tags\">\n                        " + _.reduce(filteredTags.slice(0, 5), function(memo, tag) {
                ...
                _.each(this.data, function(option) {
                    _this5.$select.append($("<option/>").attr("value", option.value).html(_.escape(option.label)));
                });
            }
        ...
        update: function update(input_def) {
