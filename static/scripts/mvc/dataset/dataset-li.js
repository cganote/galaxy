define("mvc/dataset/dataset-li", ["exports", "mvc/list/list-item", "mvc/dataset/states", "ui/fa-icon-button", "mvc/base-mvc", "utils/localization"], function(exports, _listItem, _states, _faIconButton, _baseMvc, _localization) {
        ...
        /** @lends DatasetListItemView.prototype */
        {
            ...
            */
            ...
            _renderNametags: function _renderNametags() {
                var tpl = _.template(["<% _.each(_.sortBy(_.uniq(tags), function(x) { return x }), function(tag){ %>", '<% if (tag.indexOf("name:") == 0){ %>', '<span class="badge badge-primary badge-tags"><%- tag.slice(5) %></span>', "<% } %>", "<% }); %>"].join(""));
                return tpl({
    ...
    DatasetListItemView.prototype.templates = function() {
        ...
        summaryTemplates[_states2.default.PAUSED] = _baseMvc2.default.wrapTemplate(["<div>", (0, _localization2.default)('This job is paused. Use the "Resume Paused Jobs" in the history menu to resume'), "</div>", '<div class="info"><%- dataset.misc_info %></div>'], "dataset");
