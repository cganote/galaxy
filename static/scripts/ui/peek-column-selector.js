define("ui/peek-column-selector", [], function() {
    "use strict";
    ...
    function buildControlCells(count, control) {
        ...
        for (var columnIndex = 0; columnIndex < count; columnIndex += 1) {
            $cells.push(control.multiselect ? buildMultiSelectCell(control, columnIndex) : buildSingleSelectCell(control, columnIndex));
        }
        return $cells;
    }
        ...
        if (options.renameColumns) {
            $headers.addClass(RENAMABLE_HEADER_CLASS).click(function renameColumn() {
                ...
                if (newColumnName !== null && newColumnName !== prevName) {
                    ...
                    $this.text(index + (newColumnName ? "." + newColumnName : "")).data(COLUMN_NAME_DATA_KEY, newColumnName).attr("data-", COLUMN_NAME_DATA_KEY, newColumnName);
                    // fire event for new column names
                    var columnNames = $.makeArray($this.parent().children("th:not(.top-left)").map(function() {
