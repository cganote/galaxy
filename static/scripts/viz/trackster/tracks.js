define("viz/trackster/tracks", ["exports", "utils/localization", "libs/underscore", "libs/backbone", "viz/visualization", "viz/viz_views", "viz/trackster/util", "viz/trackster/slotting", "viz/trackster/painters", "viz/trackster/filters", "mvc/dataset/data", "mvc/tool/tools", "utils/config", "viz/bbi-data-manager", "ui/editable-text"], function(exports, _localization, _underscore, _backbone, _visualization, _viz_views, _util, _slotting, _painters, _filters, _data, _tools, _config, _bbiDataManager) {
    "use strict";
    ...
    var Drawable = function Drawable(view, container, obj_dict) {
        ...
        this.config = _config2.default.ConfigSettingCollection.from_models_and_saved_values(this.build_config_params(), obj_dict.prefs);
            ...
            if (num_drawables === 0) {
                ...
                if (num_feature_tracks > 1 && num_feature_tracks === this.drawables.length) {
                    ...
                    for (var filter_name in shared_filters) {
                        var filters = shared_filters[filter_name];
                        if (filters.length === num_feature_tracks) {
                            // Add new filter.
                            // FIXME: can filter.copy() be used?
                            var new_filter = new _filters2.default.NumberFilter({
                                name: filters[0].name,
        ...
        render: function render() {
            ...
            */
            ...
            $(window).bind("resize", function() {
                ...
                this.resize_timer = window.setTimeout(function() {
                    view.resize_window();
                }, 500);
            });
    ...
    extend(TracksterView.prototype, DrawableCollection.prototype, {
        ...
        trigger_navigate: function trigger_navigate(new_chrom, new_low, new_high, delay) {
            ...
            if (delay) {
                ...
                this.timer = window.setTimeout(function() {
                    _this.trigger("navigate", new_chrom + ":" + new_low + "-" + new_high);
                }, 500);
            } else {
        ...
        load_chroms: function load_chroms(url_parms) {
            ...
            $.ajax({
                ...
                success: function success(result) {
                    ...
                    // Load chroms.
                    if (result.reference) {
                        var ref_track = new ReferenceTrack(_this2);
                        _this2.add_label_track(ref_track);
                        _this2.reference_track = ref_track;
                    }
                    _this2.chrom_data = result.chrom_info;

                    _this2.chrom_select.html("");
                    _this2.chrom_select.append($('<option value="">Select Chrom/Contig</option>'));

                    for (var i = 0, len = _this2.chrom_data.length; i < len; i++) {
                        var chrom = _this2.chrom_data[i].chrom;
                        var chrom_option = $("<option>");
                        chrom_option.text(chrom);
                        chrom_option.val(chrom);
                        _this2.chrom_select.append(chrom_option);
                    }
                    if (result.prev_chroms) {
                        _this2.chrom_select.append($("<option value=\"previous\">Previous " + MAX_CHROMS_SELECTABLE + "</option>"));
                    }
                    if (result.next_chroms) {
                        _this2.chrom_select.append($("<option value=\"next\">Next " + MAX_CHROMS_SELECTABLE + "</option>"));
                    }
                    _this2.chrom_start_index = result.start_index;
        ...
        move_delta: function move_delta(delta_chrom) {
            ...
            this.redraw_on_move_fn = window.setTimeout(function() {
                view.request_redraw();
            }, 200);
            ...
            // Go to parameter exploration visualization.
            {
                ...
                on_click_fn: function on_click_fn(track) {
                    ...
                    var ok_fn = function ok_fn() {
                        ...
                        var regions;
                        var view_region = new _visualization2.default.GenomeRegion({
                            chrom: undefined.view.chrom,
                            start: undefined.view.low,
                            end: undefined.view.high
                        });
                        var bookmarked_regions = _.map($(".bookmark"), function(elt) {
    ...
    extend(TiledTrack.prototype, Drawable.prototype, Track.prototype, {
        ...
        _draw: function _draw(options) {
            ...
            $.when.apply($, tile_promises).then(function() {
                ...
                _this3.tiles_div.children(".remove").remove();
                ...
                if (tiles.length !== 0) {
                    _this3.postdraw_actions(tiles, width, w_scale, clear_after);
                }
            });
        },
            ...
            if (line_track_tiles.length > 0) {
                ...
                _.each(tiles, function(tile) {
                    if (!(tile instanceof LineTrackTile)) {
                        ...
                        _this5.draw_helper(tile.region, w_scale, {
                            force: true,
                            mode: "Coverage"
                        });
                    }
        ...
        draw_helper: function draw_helper(region, w_scale, options) {
            ...
            var get_tile_data = function get_tile_data() {
                ...
                var tile_data = _.map(drawables, function(d // Get the track data/promise.
                    ...
                    return d.data_manager.get_data(region, data_mode, resolution, _this6.data_url_extra_params);
                });

                // Get reference data/promise.
                if (_this6.view.reference_track) {
                    tile_data.push(_this6.view.reference_track.data_manager.get_data(region, mode, resolution, _this6.view.reference_track.data_url_extra_params));
                }
            ...
            $.when.apply($, get_tile_data()).then(function() {
                ...
                if (_.find(tile_data, function(d) {
                    ...
                    _this6.tile_cache.set_elt(key, undefined);
                    $.when(_this6.draw_helper(region, w_scale, options)).then(function(tile) {
                ...
                if (_this6.view.reference_track) {
                    seq_data = _this6.view.reference_track.data_manager.subset_entry(tile_data.pop(), region);
                }
                ...
                var canvas = _this6.view.canvas_manager.new_canvas();
                var tile_low = region.get("start");
                var tile_high = region.get("end");
                var width = Math.ceil((tile_high - tile_low) * w_scale) + _this6.left_offset;
                ...
                ctx.translate(_this6.left_offset, 0);
                ...
                if (tile !== undefined) {
                    _this6.tile_cache.set_elt(key, tile);
                    _this6.show_tile(tile, w_scale);
                }
            ...
            if (line_track_tiles.length === 0) {
                ...
                _.each(tiles, function(tile) {
                    ...
                    if (_.size(tile_incomplete_features) !== 0) {
                        ...
                        var new_canvas = _this7.view.canvas_manager.new_canvas();
                        var new_canvas_ctx = new_canvas.getContext("2d");
                        new_canvas.height = Math.max(tile.canvas.height, _this7.get_canvas_height(features, tile.mode, tile.w_scale, 100));
                        new_canvas.width = tile.canvas.width;
                        new_canvas_ctx.drawImage(tile.canvas, 0, 0);
                        new_canvas_ctx.translate(_this7.left_offset, 0);
                        var new_tile = _this7.draw_tile(features, new_canvas_ctx, tile.mode, tile.region, tile.w_scale, tile.seq_data);
