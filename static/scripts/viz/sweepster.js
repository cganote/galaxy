define("viz/sweepster", ["exports", "utils/localization", "libs/underscore", "libs/d3", "viz/trackster/util", "viz/visualization", "viz/trackster/tracks", "mvc/tool/tools", "mvc/dataset/data", "utils/config", "mvc/ui/icon-button"], function(exports, _localization, _underscore, _d, _util, _visualization, _tracks, _tools, _data, _config, _iconButton) {
    "use strict";
    ...
    var ToolParameterTree = Backbone.Model.extend({
        ...
        add_param: function add_param(param) {
        ...
        remove_param: function remove_param(param) {
         ...
         */
        set_tree_data: function set_tree_data() {
            ...
            var // Creates tree data recursively.
                create_tree_data = function create_tree_data(params_samples, index) {
        ...
        get_tree_params: function get_tree_params() {
            // Filter and sort parameters to get list in tree.
            return _(this.get("tool").get("inputs").where({
                in_ptree: true
         ...
         */
        get_num_leaves: function get_num_leaves() {
         ...
         */
        get_node_settings: function get_node_settings(target_node) {
            ...
            var get_settings = function get_settings(node, settings) {
                ...
                if (!node.children) {
                    ...
                    return _.flatten(_.map(node.children, function(c) {
         ...
         */
        get_connected_nodes: function get_connected_nodes(node) {
            var get_subtree_nodes = function get_subtree_nodes(a_node) {
                if (!a_node.children) {
                    return a_node;
                } else {
                    // At interior node: return subtree nodes.
                    return _.flatten([a_node, _.map(a_node.children, function(c) {
         ...
         */
        get_leaf: function get_leaf(settings) {
            var cur_node = this.get("tree_data");

            var find_child = function find_child(children) {
    ...
    var SweepsterTrack = Backbone.Model.extend({
        ...
        initialize: function initialize(options) {
            ...
            if (options.track) {
                ...
                this.set("track", _tracks2.default.object_from_template(track_config, {}, null));
            }
        },

        same_settings: function same_settings(a_track) {
     ...
     */
    var SweepsterVisualization = _visualization2.default.Visualization.extend({
        defaults: _.extend({}, _visualization2.default.Visualization.prototype.defaults, {
            dataset: null,
            tool: null,
            parameter_tree: null,
            regions: null,
        ...
        initialize: function initialize(options) {
            ...
            this.set("tool", new _tools2.default.Tool(options.tool));
            this.set("regions", new _visualization2.default.GenomeRegionCollection(options.regions));
        ...
        add_track: function add_track(track) {
    ...
    var SweepsterTrackView = Backbone.View.extend({
        ...
        render: function render() {
            ...
            var run_on_dataset_button = $("<button/>").appendTo(settings_div).text("Run on complete dataset").click(function() {
                settings_div.toggle();
                self.trigger("run_on_dataset", settings);
            });

            var icon_menu = _iconButton2.default.create_icon_buttons_menu([{
            ...
            this.model.get("regions").each(function() {
                self.$el.append($("<td/>").addClass("tile").html($("<img/>").attr("src", Galaxy.root + "images/loading_large_white_bg.gif")));
            });
         ...
         */
        draw_tiles: function draw_tiles() {
            var self = this;
    ...
    var ToolInputValOrSweepView = Backbone.View.extend({
        ...
        number_input_template: '<div class="form-row-input sweep">' + '<input class="min" type="text" size="6" value="<%= min %>"> - ' + '<input class="max" type="text" size="6" value="<%= max %>">' + ' samples: <input class="num_samples" type="text" size="1" value="<%= num_samples %>">' + "</div>",
        ...
        render: function render() {
            ...
            if (input instanceof _tools2.default.IntegerToolParameter) {
                sweep_inputs_row = $(_.template(this.number_input_template)(this.model.toJSON()));
            } else if (input instanceof _tools2.default.SelectToolParameter) {
            ...
            var menu = _iconButton2.default.create_icon_buttons_menu([{
                title: (0, _localization2.default)("Add parameter to tree"),
                ...
                title: (0, _localization2.default)("Remove parameter from tree"),
                icon_class: "toggle",
                on_click: function on_click() {
    ...
    var ToolParameterTreeDesignView = Backbone.View.extend({
        ...
        render: function render() {
            ...
            var tool_form_view = new _tools2.default.ToolFormView({
    ...
    var ToolParameterTreeView = Backbone.View.extend({
        ...
        render: function render() {
            ...
            _.each(tree_params, function(param, index) {
                ...
                self.$el.append($("<div>").addClass("label").text(param.get("label")).css("left", x + center_left));
            });

            // Set up vis element.
            var vis = d3.select(this.$el[0]).append("svg").attr("width", this.width).attr("height", this.height + 30).append("g").attr("transform", "translate(40, 20)");
            ...
            var node = vis.selectAll("g.node").data(nodes).enter().append("g").attr("class", "node").attr("transform", function(d) {
                ...
                node.filter(function(d) {
                    return _.find(connected_node_ids, function(id) {
                        return id === d.id;
                    }) !== undefined;
                }).style("fill", "#f00");
    ...
    var SweepsterVisualizationView = Backbone.View.extend({
        ...
        helpText: "<div><h4>Getting Started</h4>" + "<ol><li>Create a parameter tree by using the icons next to the tool's parameter names to add or remove parameters." + "<li>Adjust the tree by using parameter inputs to select min, max, and number of samples" + "<li>Run the tool with different settings by clicking on tree nodes" + "</ol></div>",

        initialize: function initialize(options) {
            this.canvas_manager = new _visualization2.default.CanvasManager(this.$el.parents("body"));
        ...
        render: function render() {
            ...
            var close_button = _iconButton2.default.create_icon_buttons_menu([{
        ...
        run_tool_on_dataset: function run_tool_on_dataset(settings) {
            ...
            show_modal("Running " + tool_name + " on complete dataset", tool_name + " is running on dataset '" + dataset.get("name") + "'. Outputs are in the dataset's history.", {
                Ok: function Ok() {
                    hide_modal();
                }
            });
         ...
         */
        add_track: function add_track(pm_track) {
            var self = this;
            ...
            track_view.$el.hover(function() {
                ...
                d3.select(self.tool_param_tree_view.$el[0]).selectAll("g.node").filter(function(d) {
                    return _.find(connected_node_ids, function(id) {
                        return id === d.id;
                    }) !== undefined;
                }).style("fill", "#f00");
         ...
         */
        handle_node_clicks: function handle_node_clicks() {
            ...
            node.on("click", function(d, i) {
                ...
                if (all_settings.length >= 10) {
                    show_modal("Whoa there cowboy!", "You clicked on a node to try " + self.model.get("tool").get("name") + " with " + all_settings.length + " different combinations of settings. You can only run 10 jobs at a time.", {
                        Ok: function Ok() {
                            hide_modal();
                    ...
                    _.each(new_tracks, function(pm_track, index) {
                        setTimeout(function() {
                            ...
                            $.when(tool.rerun(dataset, regions)).then(function(output) {
                                ...
                                var track_obj = _tracks2.default.object_from_template(track_config, self, null);
